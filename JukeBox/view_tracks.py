...
class TrackViewer():
    def __init__(self, window):
        ...
        lib.load_from_csv()                 # save and load using csv
        ...
        self.image_label = tk.Label(window)  # placeholder for track image
    ...
    def view_tracks_clicked(self):
        ...
        if name is not None:
            ...
            self.load_image(key)  # load and display the image
        else:
    ...
    def load_image(self, key):
        try:
            # attempt to load the image corresponding to the track ID
            img_path = f"images/{key}.jpg"# images stored as <track_id>.jpg
            img = Image.open(img_path)
            img = img.resize((150, 150))  # resize to fit the UI
            img = ImageTk.PhotoImage(img) # converts to a format compatible with tkinter
            self.image_label.config(image=img)
            self.image_label.image = img  # keep a reference to avoid garbage collection
        except FileNotFoundError:
            self.image_label.config(image="")   # clears the image if not found
